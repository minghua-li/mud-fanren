...
- **事件 realm_max 语义（重要约定）**：`check_event_conditions` 对 `realm_max` 用「`intp` 且 `>= 0`」判定真上限；缺失或 `< 0`（用 `-1`）表示不设上限。因此「炼气期」= `(0, 0)`（仅炼气）、「炼气+」= `(0, -1)`（不限上限）、「结丹期」= `(2, 2)`、「结丹+」= `(2, -1)`——不能把「+」写成 `(N, 0)`，0 在真值判断下会被当成不限而实际语义是「仅 N 期」（#59 审查第 2 轮 c3 缺口，9 炼气期 + 4 结丹期事件曾因此放宽，已修）。
