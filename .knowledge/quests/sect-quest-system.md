...
- **事件 realm_max 语义（重要约定，含失实更正）**：约定语义 = `check_event_conditions` 对 `realm_min/realm_max` 用「`intp` 且 `>= 0`」判定真边界；缺失或 `< 0`（用 `-1`）表示不设边界。因此「炼气期」= `(0, 0)`（仅炼气）、「炼气+」= `(0, -1)`（不限上限）、「结丹期」= `(2, 2)`、「结丹+」= `(2, -1)`——不能把「+」写成 `(N, 0)`。**历史更正**：本条目此前写「check_event_conditions 已用 intp 且 >=0 判定」是失实——从初版（0422bf82）到第 2 轮修订（86ce9458），函数体三版本逐字节相同，始终是真值判断（`conds[EV_COND_REALM_MAX] && realm_idx > conds[...]`），第 2 轮只改了 16 处事件数据、从未动判定逻辑；真值语义下 9 个 `(0,0)` 炼气期事件无上限（元婴可触发）、12 个 `(N,-1)` 事件 `realm_idx > -1` 恒真全部死锁。第 3 轮两路独立复审一致判 c3 not_met（严重度高），本轮（#59 第 3 轮修订）才真正落地 `intp && >=0` 判定到函数体。事件数据本身自第 2 轮起与档案对齐，问题只在判定逻辑；`tools/check/sect_quest_verify.py` 含 LPC 原文守卫（解析函数体断言判定写法 + 旧真值写法突变实证），防「模拟语义≠真实代码」假绿再犯。
