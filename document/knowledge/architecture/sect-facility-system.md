...
- 消耗铁律：**先查贡献（SECT_D->query_contribution）后扣灵石（MONEY_D->player_pay），最后 SECT_D->add_contribution(负值)**——顺序颠倒会让贡献不足的玩家白扣灵石（path_verify.py 突变验证过红→绿）。贡献奖励（演武场切磋/每日修行）用 add_contribution(正值, "设施修行：<设施名>")。
- buff 存玩家属性 `sect_facility/buffs`（key→({expire,value})），过期查询返回 0；对外钩子 query_danfang_bonus / query_forge_bonus / query_training_bonus / query_defense_bonus 供炼丹/炼器/打坐/战斗系统接入，同类型多设施取最大值（query_effect_value）。
...
- 数值对齐来源：02-扩充内容/02-区域游戏玩法.md §4.4（炼丹+10%/传功堂+10%/护山大阵 30000/15000、100000/50000）与 mansion.h（PLOT_*/GARDEN_MAINTENANCE/LAND_*）。
- 端到端验证：`python3 tools/facility/path_verify.py`（47 断言，数据驱动解析 LPC 配置，覆盖入宗→种植收获→丹房加成→藏经阁→坊市→每日修行→升级→限额→拦截）。
