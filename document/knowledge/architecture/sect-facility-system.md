...
- 消耗铁律：**先查贡献（SECT_D->query_contribution）后扣灵石（MONEY_D->player_pay），最后 SECT_D->add_contribution(负值)**——顺序颠倒会让贡献不足的玩家白扣灵石。该顺序由 path_verify.py 的「LPC 原文守卫」直接解析 sect_facility_d.c:pay_cost 函数体做机器验证（含 LPC 原文突变实证：颠倒顺序→守卫转红），不仅停留在 Python 翻译层。贡献奖励（演武场切磋/每日修行）用 add_contribution(正值, "设施修行：<设施名>")。
...
- 验证（双层，均入库可复跑）：① `python3 tools/facility/static_check.py`（71 断言静态验收：括号状态机/字符串截断/exits 双向/接口签名/设施配置↔房间一致）；② `python3 tools/facility/path_verify.py`（51 断言：Python 翻译层端到端 + LPC 原文守卫——pay_cost 扣费顺序与 base_name 房间匹配直接解析 LPC 原文断言，含 LPC 原文突变实证）。注意分层语义：Python 翻译层验证的是翻译镜像对 LPC 配置字面量的忠实性；LPC 原文守卫验证的是原文关键逻辑顺序——两者都不等于执行 LPC（环境无 fluffos driver），运行时行为仍需人工装驱动复验。
