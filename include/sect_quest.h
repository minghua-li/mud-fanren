...
// ─── 宗门事件触发条件键 ───
#define EV_COND_REALM_MIN      "realm_min"       // 最低境界索引（intp 且 >=0 为真下限；<0（-1）为不设下限；缺失键 LPC 取 0 等同显式 0）
#define EV_COND_REALM_MAX      "realm_max"       // 最高境界索引（intp 且 >=0 为真上限；<0（-1）为不限，(N,-1)=「N 期以上不限」；缺失键 LPC 取 0 等同显式 0=仅 N 期）
